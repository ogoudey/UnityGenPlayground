...
class Coordinator(Agent):    
    ...
    instructions_v2= """You are the Coordinator agent responsible for generating a Unity scene that follows the user plan. 
...
- Use planandplaceGround multiple times if need be.
...
Your role is to reliably build a coherent, grounded (Unity) world from the description."""
    ...
    instructions_v3= {"o3-mini":"""You are the Coordinator agent responsible for generating a Unity scene that matches the user prompt. 
You must orchestrate tool usage in the following structured order:
...
Your role is to reliably build a coherent, grounded Unity world from the description.""",
"gpt-5-mini":"""You are the Coordinator agent responsible for generating a Unity scene that matches the user prompt. 
You must orchestrate tool usage in the following structured order:
...
Your role is to reliably build a coherent, grounded Unity world from the description."""}
    ...
    def __init__(self, name=None, instructions=None, tools=None):
        super().__init__(
            ...
            instructions=instructions or Coordinator.instructions_v3[MODEL],
            tools=tools or [get_contact_points, place_vr_human_player, planSkybox, placeSkybox, planandplaceGround, planObject, placeObject, planandplaceSun],
            model=MODEL,
        )
